...
namespace jc 
{
	...
	void Shape::Draw(Core::Graphics& graphics, const Vector2& position,float angle ,float scale)
	{
		graphics.SetColor(color);
			...
			{
				jc::Vector2 p1 = position + (Vector2::Rotate(points[i],angle) * scale);
				jc::Vector2 p2 = position + (Vector2::Rotate(points[i + 1],angle) * scale);
	...
	void Shape::Draw(Core::Graphics& graphics, const Transform& transform)
	{
		...
		for (size_t i = 0; i < points.size() - 1; i++)
		{
			jc::Vector2 p1 =  transform.matrix * points[i];
			jc::Vector2 p2 =  transform.matrix * points[i+1];
